...
export function useRunawayButtons(buttonCount: number = 3) {
  ...
  function moveButton(index: number, mouseX: number, mouseY: number, containerWidth: number, containerHeight: number) {
      ...
      for (let i = 0; i < buttons.value.length; i++) {
        if (i !== index) {
          ...
          if (isOverlapping(tempButton, other)) {
            ...
            else newX = Math.min(other.x + other.width + 5, containerWidth - button.width)

            if (isOverlapping({ ...button, x: newX, y: newY }, other)) {
              if (dy >= 0) newY = Math.max(other.y - button.height - 5, 0)
              else newY = Math.min(other.y + other.height + 5, containerHeight - button.height)
            }
          }
        }
      }
